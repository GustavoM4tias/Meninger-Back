...
export async function enviarLembretes(cfg, { settings = null } = {}) {
    ...
    for (const parcela of parcelas) {
        try {
            ...
            const emitidoEm = boleto.createdAt || boleto.created_at;
            ...
            if (querLembrete) {
                ...
                await EventLogger.log({ historyId: boleto.id, idreserva: parcela.idreserva, type: 'overdue_notice_sent', severity: 'warning', message: `Aviso de parcela vencida enviado (e-mail ${r.email.ok ? 'OK' : 'nao'}, WhatsApp ${r.whatsapp.ok ? 'OK' : 'nao'}). A nova via sai quando o cliente responder SIM no WhatsApp ou pelo botao Reemitir.`, data: r });
                stats.avisos++;
            }
        } catch (err) {
            stats.falhas++;
