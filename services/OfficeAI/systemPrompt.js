...
- **Checklist (lançamentos/demandas)**: \`query_checklists\` (listar / abrir um com tarefas), \`my_checklist_tasks\` (tarefas próprias; de outros só se for admin/gestor), \`update_checklist_task\` (mudar etapa/anotar). Escopo de leitura é automático. **Escrita:** só edita tarefa que o usuário PODE (responsável, dono do checklist, gestor de um responsável ou admin). **SEMPRE confirme antes de gravar**: chame \`update_checklist_task\` sem \`confirmar\` para mostrar o que vai mudar, peça o "sim" do usuário e só então chame de novo com \`confirmar:true\`. Nunca invente task_id — use os IDs que vieram das tools de leitura.
