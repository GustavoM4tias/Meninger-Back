...
const PRE_SYNC_STATEMENTS = [
    ...
    // Wiki-link — apelidos do artigo para auto-link nos demais artigos.
    `ALTER TABLE academy_articles ADD COLUMN IF NOT EXISTS aliases JSONB NOT NULL DEFAULT '[]'::jsonb`,

    // Subcategoria (2º nível da KB): Comercial > Cartório > artigo.
    `ALTER TABLE academy_articles ADD COLUMN IF NOT EXISTS subcategory_slug VARCHAR(255)`,

    // ── Modelo multi-audience (substitui o enum legacy `audience`) ────────
    // Cada artefato passa a carregar um SET de tokens de público:
    //   INTERNAL, GESTOR, ADMIN, BROKER, REALESTATE, CORRESPONDENT
    // O usuário também tem um set de tokens; ele vê o artefato se houver
    // interseção (operador jsonb ?| ARRAY[...] no Postgres).
    `ALTER TABLE academy_articles  ADD COLUMN IF NOT EXISTS audiences JSONB NOT NULL DEFAULT '[]'::jsonb`,
    `ALTER TABLE academy_tracks    ADD COLUMN IF NOT EXISTS audiences JSONB NOT NULL DEFAULT '[]'::jsonb`,
    `ALTER TABLE academy_topics    ADD COLUMN IF NOT EXISTS audiences JSONB NOT NULL DEFAULT '[]'::jsonb`,
    `ALTER TABLE academy_highlights ADD COLUMN IF NOT EXISTS audiences JSONB NOT NULL DEFAULT '[]'::jsonb`,

    // Permissão de edição por artigo (além do autor + admin).
    `ALTER TABLE academy_articles ADD COLUMN IF NOT EXISTS editor_user_ids JSONB NOT NULL DEFAULT '[]'::jsonb`,

    // Visibilidade por DEPARTAMENTO (modelo interno do Office) — substitui a
    // "audiência" no editor. [] = GERAL (todos); [ids de Department] = só esses
    // departamentos (+ admin) veem. Convive com `audiences` (dormant) no banco.
    `ALTER TABLE academy_articles ADD COLUMN IF NOT EXISTS department_ids JSONB NOT NULL DEFAULT '[]'::jsonb`,
    `ALTER TABLE academy_tracks   ADD COLUMN IF NOT EXISTS department_ids JSONB NOT NULL DEFAULT '[]'::jsonb`,

    // ── Eme × Processos: digest + grafo + embedding (economia de tokens) ──
    // ai_digest    = resumo estruturado por artigo (gerado 1× no publish, Flash).
    // process_meta = grafo (processFor/prerequisites) — autor e/ou AI-extraction.
    // digest_hash  = hash do corpo → regenera digest só quando o conteúdo muda.
    // embedding    = pgvector p/ busca semântica. CREATE EXTENSION pode falhar
    //   sem privilégio/extensão no Railway → busca degrada p/ keyword (ok).
    `CREATE EXTENSION IF NOT EXISTS vector`,
    `ALTER TABLE academy_articles ADD COLUMN IF NOT EXISTS ai_digest JSONB`,
    `ALTER TABLE academy_articles ADD COLUMN IF NOT EXISTS process_meta JSONB`,
    `ALTER TABLE academy_articles ADD COLUMN IF NOT EXISTS digest_hash VARCHAR(64)`,
    `ALTER TABLE academy_articles ADD COLUMN IF NOT EXISTS embedding vector(768)`,

    // Menções @ no Academy: usuário "protegido" (mentionable=false) não é
    // mencionável por não-admins. Default true = todo mundo mencionável.
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS mentionable BOOLEAN NOT NULL DEFAULT true`,

    // Backfill: deriva audiences[] do enum `audience` legacy onde ainda estiver vazio.
    //   BOTH        → todos os públicos (exceto ADMIN — admin sempre vê tudo via override)
    //   GESTOR_ONLY → ["GESTOR"]
    //   ADM_ONLY    → ["ADMIN"]
    `UPDATE academy_articles SET audiences = CASE
        WHEN audience = 'GESTOR_ONLY' THEN '["GESTOR"]'::jsonb
        WHEN audience = 'ADM_ONLY'    THEN '["ADMIN"]'::jsonb
        ELSE '["INTERNAL","GESTOR","BROKER","REALESTATE","CORRESPONDENT"]'::jsonb
       END
       WHERE jsonb_array_length(audiences) = 0`,
    `UPDATE academy_tracks SET audiences = CASE
        WHEN audience = 'GESTOR_ONLY' THEN '["GESTOR"]'::jsonb
        WHEN audience = 'ADM_ONLY'    THEN '["ADMIN"]'::jsonb
        ELSE '["INTERNAL","GESTOR","BROKER","REALESTATE","CORRESPONDENT"]'::jsonb
       END
       WHERE jsonb_array_length(audiences) = 0`,
    `UPDATE academy_topics SET audiences = CASE
        WHEN audience = 'GESTOR_ONLY' THEN '["GESTOR"]'::jsonb
        WHEN audience = 'ADM_ONLY'    THEN '["ADMIN"]'::jsonb
        ELSE '["INTERNAL","GESTOR","BROKER","REALESTATE","CORRESPONDENT"]'::jsonb
       END
       WHERE jsonb_array_length(audiences) = 0`,
    `UPDATE academy_highlights SET audiences = CASE
        WHEN audience = 'GESTOR_ONLY' THEN '["GESTOR"]'::jsonb
        WHEN audience = 'ADM_ONLY'    THEN '["ADMIN"]'::jsonb
        ELSE '["INTERNAL","GESTOR","BROKER","REALESTATE","CORRESPONDENT"]'::jsonb
       END
    ...
    // ── Modelo de VISIBILIDADE em 4 classes (2026-06) ─────────────────────
    // Materiais curados (artigos, trilhas, highlights) são canonicalizados
    // para exatamente UM dos 4 sets:
    //   INTERNO  = ["INTERNAL","GESTOR"]
    //   EXTERNO  = ["BROKER","REALESTATE","CORRESPONDENT"]
    //   AMBOS    = união dos dois
    //   ADMIN    = ["ADMIN"]
    // Regra: interno+externo → AMBOS; só interno → INTERNO; só externo →
    // EXTERNO; só ADMIN → ADMIN. Idempotente (canônico mapeia nele mesmo).
    // Tópicos da comunidade ficam FORA (user-generated, clampados ao criador).
    ...['academy_articles', 'academy_tracks', 'academy_highlights'].map((t) => `
        UPDATE ${t} SET audiences = CASE
            WHEN (audiences ?| ARRAY['INTERNAL','GESTOR'])
             AND (audiences ?| ARRAY['BROKER','REALESTATE','CORRESPONDENT'])
                THEN '["INTERNAL","GESTOR","BROKER","REALESTATE","CORRESPONDENT"]'::jsonb
            WHEN audiences ?| ARRAY['INTERNAL','GESTOR']
                THEN '["INTERNAL","GESTOR"]'::jsonb
            WHEN audiences ?| ARRAY['BROKER','REALESTATE','CORRESPONDENT']
                THEN '["BROKER","REALESTATE","CORRESPONDENT"]'::jsonb
            WHEN audiences ? 'ADMIN'
                THEN '["ADMIN"]'::jsonb
            ELSE audiences
        END
        WHERE jsonb_array_length(audiences) > 0`),
...
const POST_SYNC_STATEMENTS = [
    ...
    // ─── Eme × Processos: índice ANN p/ busca semântica (hnsw, cosseno) ────
    // Falha silenciosa se pgvector/coluna `embedding` não existir → a busca
    // semântica degrada para keyword sem quebrar o boot.
    `CREATE INDEX IF NOT EXISTS academy_articles_embedding_hnsw
        ON academy_articles USING hnsw (embedding vector_cosine_ops)`,
];

// pgvector é opcional: sem a extensão disponível no servidor Postgres, a busca
// semântica do Academy degrada para keyword (comportamento previsto). Checamos
// a disponibilidade UMA vez e pulamos os statements dependentes com um aviso
// claro — em vez de deixar o CREATE EXTENSION estourar um erro em todo boot.
...
async function pgvectorAvailable() {
    ...
    if (!_pgvectorAvailable) {
        console.log('ℹ️  [AcademySchema] pgvector indisponível neste Postgres — busca semântica do Academy segue em modo keyword (previsto). Para habilitar, use uma imagem Postgres com a extensão pgvector.');
    }
    return _pgvectorAvailable;
}

const isVectorStatement = (sql) => /vector/i.test(sql);

async function runBatch(statements, label) {
    let applied = 0;
    let skipped = 0;
    const hasVector = await pgvectorAvailable();
    for (const sql of statements) {
        if (!hasVector && isVectorStatement(sql)) { skipped++; continue; }
        try {
