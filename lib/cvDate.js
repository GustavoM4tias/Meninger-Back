...
/**
 * Trechos de SQL para colunas timestamptz que guardam instante do CV.
 *
 * Depois que a gravacao passou a ser o instante certo, `DATE(col)` e
 * `col BETWEEN '... 00:00:00' AND '... 23:59:59'` passam a cortar o dia em UTC
 * (21h de Brasilia) - lead das 21h-24h cairia no dia seguinte. Aqui o corte e
 * sempre no dia de Brasilia, e o BETWEEN continua usando o indice da coluna.
 *
 * `start`/`end` sao placeholders de parede de Brasilia ('YYYY-MM-DD HH:mm:ss').
 */
export const sqlDiaCv = (col) => `(${col} AT TIME ZONE '${FUSO_CV}')::date`;
export const sqlMesCv = (col) => `to_char(${col} AT TIME ZONE '${FUSO_CV}', 'YYYY-MM')`;
export const sqlEntreCv = (col, start = ':start', end = ':end') =>
    `${col} BETWEEN (CAST(${start} AS timestamp) AT TIME ZONE '${FUSO_CV}') `
    + `AND (CAST(${end} AS timestamp) AT TIME ZONE '${FUSO_CV}')`;

export default { parseCvDate, formatCvDate, FUSO_CV, sqlDiaCv, sqlMesCv, sqlEntreCv };
