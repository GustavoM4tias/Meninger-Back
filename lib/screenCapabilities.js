...
export const SCREEN_CAPABILITIES = {
    ...
    '/microsoft/outlook': {
        ...
        // A IA da caixa. Ler o que ela entendeu vai junto de 'view'; MEXER no
        // que ela pode fazer sozinha (contexto, nivel de permissao, regras,
        // assuntos protegidos, teto de valor) e acao propria - e a diferenca
        // entre alguem que usa a triagem e alguem que autoriza a IA a agir.
        // Aprovar um rascunho da fila NAO entra aqui: aprovar e enviar, e
        // enviar ja tem capacidade propria ('send').
        automate: 'screen',
    },
    ...
    // Validador de Contratos — subir os dois PDFs e ler o histórico é o uso
    // normal de quem confere contrato, e junto vai o FAROL: saber que o
    // validador está fora do ar antes de subir o PDF poupa a pessoa de
    // interpretar um erro genérico. O farol é só cor, desde-quando e uma frase.
    //
    // O DETALHE da saúde (quais modelos responderam, mensagem de erro do
    // provedor, endereço do gatilho) e a CONFIGURAÇÃO (pool de modelos, ritmo
    // da sonda, prazos, destinatários do alerta) ficam em 'configure': mexem na
    // ferramenta inteira e na conta do provedor, e o endereço do webhook é
    // segredo. Isto mantém o acesso que a tela já tinha - nada foi alargado.
    '/validator': {
        view: 'screen',       // validar par de PDFs, histórico, farol de saúde
        configure: 'admin',   // detalhe da sonda, testar agora, modelos, alertas
    },

    // Processos da Eme - o mapa de como a empresa trabalha, o que o motor
    // observou e o que ele quer propor.
    //
    // A tela NAO e travada em codigo de proposito. Quem entende o processo
    // comercial e o gestor comercial, nao o administrador do sistema; travar
    // aqui faria toda regra esperar o admin e o laco de aprendizado morreria na
    // segunda semana. A trava, se for preciso, e o cadeado da tela de Alcadas.
    '/tools/eme-processos': {
        view: 'screen',        // ver o mapa, a fila de propostas e o historico

        // Aprovar e o ato que transforma "o motor achou" em "a empresa faz
        // assim". Fica com quem tem a tela: dar a tela ja e dizer "voce e dono
        // deste processo", e separar as duas coisas so criaria um dono que nao
        // pode decidir nada.
        aprovar: 'screen',

        // Mudar o degrau de autonomia e ADMIN, e esta e a linha que importa:
        // aprovar uma regra diz o que e certo fazer; subir a autonomia diz que
        // a IA passa a fazer sozinha. Sao decisoes de peso diferente, e quem
        // assume a segunda e quem responde pelo sistema.
        autonomia: 'admin',

        // O portao da fila (evidencia minima, largura para virar regra da
        // empresa, teto do dia). Afrouxar isso enfraquece a trava contra
        // vazamento pela regra, entao nao e configuracao de operacao.
        configurar: 'admin',
    },

    // Aditivos — acompanhamento das assinaturas. Ver a lista (com o link para
    // reenviar ao cliente) é o uso normal de quem cobra assinatura. Releitura
    // no DocuSign é 'operate' porque bate na API deles a cada clique.
    '/comercial/aditivos': {
        view: 'screen',
        operate: 'screen',
    },
    ...
    // Frota — o veículo corporativo. Reservar é o uso normal de quem tem a
    // tela; cadastrar veículo e mexer na configuração é administração.
    //
    // Bloquear por manutenção e cancelar a reserva de OUTRA pessoa não estão
    // aqui de propósito: são o gestor da frota, que é papel do módulo
    // (fleet_settings.gestor_user_ids) e não alçada x admin. Esta tabela só
    // sabe responder as duas últimas, e aceitar um terceiro papel aqui a
    // transformaria num segundo motor de regras.
    '/frota': {
        view: 'screen',       // ver a agenda e quem está com o carro
        reservar: 'screen',   // reservar, retirar, devolver, cancelar a própria
        configurar: 'admin',  // cadastrar veículo, prazos, evento, webhook
    },
    ...
    '/crm/buildings': {
        ...
        configure: 'admin',   // espelho de vendas: faces, dormitórios, dígitos do número
    },
    ...
    '/crm/imobiliarias': {
        ...
        configure: 'admin',   // credencial do painel do CV (v3) e quem é avisado quando ela cai
    },

    // Stand de Vendas — ver o stand do empreendimento e o custo dele é trabalho
    // do dia (o QUE cada um enxerga já é cortado por grant de empreendimento no
    // accessScopeService). CUIDAR do stand — cadastrar, classificar o gasto
    // entre construção e recorrência, marcar os itens, subir foto, definir — é
    // ação própria, de quem responde por aquele stand. Já os MODELOS e as
    // CATEGORIAS de gasto valem para todos os stands da empresa: mexer neles
    // muda o número da tela de todo mundo, então é admin.
    ...
    // Projeção de Investimentos — os números vêm de uma planilha do SharePoint.
    // Reler o arquivo é trabalho de quem acompanha o número; trocar QUAL
    // planilha e a régua de atenção/estouro muda a tela de todo mundo: admin.
    '/marketing/projecao-investimentos': {
        view: 'screen',
        refresh: 'screen',
        configure: 'admin',
    },
