...
const STATEMENTS = [
    ...
    // Antes de emitir cobranca nova do ato, reconsultar no Ecobranca o ultimo
    // boleto cancelado por "BAIXADO POR DEVOLUCAO" (pode ser pagamento).
    `ALTER TABLE boleto_settings ADD COLUMN IF NOT EXISTS reconsultar_baixado_antes_emitir BOOLEAN NOT NULL DEFAULT TRUE`,
    // "BAIXADO POR DEVOLUCAO" sem baixa pedida pelo Office pode ser pagamento
    // em compensacao: o boleto fica pendente por N dias uteis antes de cancelar
    // (avaliarBaixaAmbigua). A primeira vista fica anotada no historico.
    `ALTER TABLE boleto_settings ADD COLUMN IF NOT EXISTS baixa_devolucao_confirmar_dias_uteis INTEGER DEFAULT 0`,
    `ALTER TABLE boleto_history ADD COLUMN IF NOT EXISTS baixa_devolucao_vista_em TIMESTAMP WITH TIME ZONE`,
    ...
    // Vencimento acima do limite agenda em vez de rejeitar (reserva 8294, 24/08/2026).
    `ALTER TABLE boleto_settings ADD COLUMN IF NOT EXISTS agendar_vencimento_acima_limite BOOLEAN NOT NULL DEFAULT TRUE`,
    ...
    `ALTER TABLE boleto_settings ADD COLUMN IF NOT EXISTS parcelas_idseries TEXT DEFAULT '[20]'`,
    `ALTER TABLE boleto_settings ADD COLUMN IF NOT EXISTS parcelas_exigir_ato_pago BOOLEAN NOT NULL DEFAULT TRUE`,
    `ALTER TABLE boleto_settings ADD COLUMN IF NOT EXISTS parcelas_empreendimentos_excluidos JSONB DEFAULT '[]'::jsonb`,
    ...
    `ALTER TABLE boleto_settings ADD COLUMN IF NOT EXISTS aviso_final_sem_resposta_dias INTEGER DEFAULT 15`,
    `ALTER TABLE boleto_settings ADD COLUMN IF NOT EXISTS parcelas_aviso_encerramento BOOLEAN NOT NULL DEFAULT TRUE`,
    // parcelas_contato foi criada em 09/09/2026 e ficou orfa no mesmo dia: o aviso
    // final manda "procurar o corretor", sem numero. Nao e lida.
    `ALTER TABLE ato_parcelas ADD COLUMN IF NOT EXISTS aviso_final_enviado_em TIMESTAMP WITH TIME ZONE`,
    `ALTER TABLE boleto_settings ADD COLUMN IF NOT EXISTS parcelas_ultima_rodada_em TIMESTAMP WITH TIME ZONE`,
    `ALTER TABLE boleto_settings ADD COLUMN IF NOT EXISTS check_ultima_rodada_em TIMESTAMP WITH TIME ZONE`,
    ...
    // CEP recusado pela Caixa: boleto sai com o endereco da Menin e a reserva
    // fica com alerta para corrigir o CV (08/09/2026).
    `ALTER TABLE boleto_settings ADD COLUMN IF NOT EXISTS parcelas_cep_contingencia_ativo BOOLEAN NOT NULL DEFAULT TRUE`,
    `ALTER TABLE boleto_settings ADD COLUMN IF NOT EXISTS parcelas_cep_contingencia JSONB DEFAULT '{"cep":"17500005","endereco":"Rua São Luiz","numero":"231","complemento":"","bairro":"Centro","cidade":"Marília","estado":"SP"}'::jsonb`,
    `ALTER TABLE ato_planos ADD COLUMN IF NOT EXISTS cadastro_alerta TEXT`,
    // Parcela identificada ao cliente pelo MES do vencimento em vez de "3 de 60"
    // (10/09/2026): quem tem retroativa nunca cobrada nao pode ler o numero da
    // sequencia. Ver models/sequelize/boleto/atoPlano.js.
    `ALTER TABLE ato_planos ADD COLUMN IF NOT EXISTS numeracao_oculta BOOLEAN NOT NULL DEFAULT FALSE`,
...
const PATCHES_UNICOS = [
    {
        // 10/09/2026 (Gustavo): no Inga, 19 planos tem parcela vencida que o
        // Office nunca cobrou (retroativa anterior ao corte de 08/09) e que fica
        // como esta - a rodada segue emitindo so as proximas. Para esses o texto
        // ao cliente nao pode dizer "parcela 3 de 60": o numero entrega uma
        // divida de 1 e 2 que ninguem cobrou dele. Passam a ser identificadas
        // pelo mes ("parcela de outubro/2026"). Os demais seguem numerando.
        // A tela liga e desliga plano a plano depois disso.
        key: 'ato.parcelas.numeracao_por_mes_inga_2026_09_10',
        // NOTA: o patch seguinte estendeu a regra para TODO plano ATIVO no mesmo
        // caso (Moacir Marangoni, Park Alameda, Anjos). Este ficou porque ja
        // rodou; a regra viva e a de baixo.
        sql: `
            UPDATE ato_planos p
               SET numeracao_oculta = TRUE, updated_at = NOW()
             WHERE p.empreendimento = 'RESIDENCIAL INGÁ'
               AND p.status IN ('ativo', 'pausado')
               AND EXISTS (
                   SELECT 1 FROM ato_parcelas x
                    WHERE x.plano_id = p.id AND x.status IN ('prevista', 'erro')
                      AND x.vencimento < COALESCE(
                          (SELECT parcelas_cobrar_a_partir_de FROM boleto_settings WHERE id = 1),
                          DATE '2026-09-08'))`,
    },
    {
        // 10/09/2026 (Gustavo, depois de medir): o caso do Inga existe em todo
        // lugar - 67 planos vivos tem parcela vencida que o Office nunca cobrou.
        // Decisao: vale para os ATIVOS (cliente sendo cobrado agora), 15 alem do
        // Inga - Moacir Marangoni 10, Park Alameda 3, Anjos 2. Os PAUSADOS ficam
        // como estao ("os pausados nao mexe"): Sarandi 25, Santa Stella 5, Viva
        // Sul 3 nao cobram ninguem hoje. Se voltarem a cobrar, e decisao nova.
        key: 'ato.parcelas.numeracao_por_mes_ativos_2026_09_10',
        sql: `
            UPDATE ato_planos p
               SET numeracao_oculta = TRUE, updated_at = NOW()
             WHERE p.status = 'ativo'
               AND COALESCE(p.origem, '') <> 'teste'
               AND NOT p.numeracao_oculta
               AND EXISTS (
                   SELECT 1 FROM ato_parcelas x
                    WHERE x.plano_id = p.id AND x.status IN ('prevista', 'erro')
                      AND x.vencimento < COALESCE(
    ...
    {
        // 09/09/2026 (Gustavo): so a serie 20 (Recurso Proprio Parcelado) e do
        // Office - PARCELAS MENSAIS (1) e URBAN (37) saem. Vias novas por parcela
        // caem de 3 para 2 (= dois avisos com oferta de via; depois sai o aviso
        // final). O unico plano vivo fora da serie 20 (Verona, reserva 8359, 24
        // parcelas da serie 1, nada emitido) encerra como MANUAL: a tela reabre
        // se a decisao mudar.
        key: 'ato.parcelas.so_recurso_proprio_2_avisos_2026_09_09',
        sql: `
            WITH cfg AS (
                UPDATE boleto_settings SET parcelas_idseries = '[20]', atraso_max_reemissoes = 2 WHERE id = 1 RETURNING id
            ), planos AS (
                UPDATE ato_planos p
                   SET status = 'encerrado', encerrado_motivo = 'manual',
                       encerrado_detalhe = 'serie fora da cobranca do Office (so Recurso Proprio Parcelado, decisao de 09/09/2026)',
                       encerrado_em = NOW(), updated_at = NOW()
                 WHERE p.status IN ('ativo', 'pausado')
                   AND NOT EXISTS (SELECT 1 FROM ato_parcelas x WHERE x.plano_id = p.id AND x.idserie = 20)
                   AND EXISTS (SELECT 1 FROM ato_parcelas x WHERE x.plano_id = p.id AND x.idserie IN (1, 37))
                RETURNING p.id
            )
            UPDATE ato_parcelas x SET status = 'cancelada', updated_at = NOW()
              FROM planos pl WHERE x.plano_id = pl.id AND x.status IN ('prevista', 'erro', 'vencida')`,
    },
    {
    ...
    {
        // 08/09/2026 (noite, Gustavo): o TITULO no Sienge nao e criterio (pode ser
        // adiantamento); so a venda faturada e a etapa do repasse encerram. Os
        // planos pausados na ativacao "por ter titulo" voltam a ativo, EXCETO o
        // Park Alameda Sarandi, parado por decisao da construtora (ganha o motivo).
        key: 'ato.parcelas.regra_so_repasse_2026_09_08',
        sql: `
            WITH sarandi AS (
                UPDATE ato_planos
                   SET encerrado_detalhe = 'empreendimento sem cobranca antes da assinatura do financiamento (decisao da construtora, 08/09/2026)',
                       observacao = NULL, updated_at = NOW()
                 WHERE status = 'pausado' AND observacao LIKE 'PAUSADO na ativacao de 08/09/2026%'
                   AND upper(coalesce(empreendimento,'')) LIKE '%SARANDI%'
                RETURNING id
            )
            UPDATE ato_planos
               SET status = 'ativo', pausado_em = NULL, pausado_por = NULL, observacao = NULL, updated_at = NOW()
             WHERE status = 'pausado' AND observacao LIKE 'PAUSADO na ativacao de 08/09/2026%'
               AND upper(coalesce(empreendimento,'')) NOT LIKE '%SARANDI%'`,
    },
    {
    ...
    {
        // 15/09/2026 (Gustavo): a releitura de boleto ja cancelado cai de 7 para
        // 3 dias. A situacao ambigua agora e tratada ANTES de cancelar
        // (baixa_devolucao_confirmar_dias_uteis), entao a janela e so a rede
        // de seguranca e nao precisa reler tanto - cada leitura e uma entrada
        // no portal. Vale so para quem ainda esta com o 7 gravado.
        key: 'boleto.revalidacao.3dias_2026_09',
        sql: `UPDATE boleto_settings SET revalidacao_baixado_dias = 3 WHERE id = 1 AND revalidacao_baixado_dias = 7`,
    },
    {
        // 24/09/2026 (Gustavo, reserva 8488): "baixado por devolucao" baixa na
        // primeira leitura e manda a mensagem de sempre. A janela de
        // revalidacao (revalidacao_baixado_dias) segue relendo o boleto
        // cancelado todo dia; se aparecer pago, vira pago no Office e no CV.
        // Vale so para quem ainda esta com o 3 gravado.
        key: 'boleto.baixa_devolucao.baixa_imediata_2026_09',
        sql: `ALTER TABLE boleto_settings ALTER COLUMN baixa_devolucao_confirmar_dias_uteis SET DEFAULT 0;
              UPDATE boleto_settings SET baixa_devolucao_confirmar_dias_uteis = 0 WHERE id = 1 AND baixa_devolucao_confirmar_dias_uteis = 3`,
    },
    {
