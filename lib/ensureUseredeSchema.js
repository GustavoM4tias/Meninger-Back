...
const STATEMENTS = [
    ...
    `CREATE TABLE IF NOT EXISTS userede_link_history (
        ...
        comissao_percentual_aplicada DECIMAL(6,2),
    ...
    `ALTER TABLE userede_link_history ADD COLUMN IF NOT EXISTS comissao_percentual_aplicada DECIMAL(6,2)`,
    // Comissão fora do contrato deduzida da série, pelo número do CV. Mesma
    // regra do boleto (ver lib/ensureBoletoSchema.js).
    `ALTER TABLE userede_link_history ADD COLUMN IF NOT EXISTS comissao_valor_deduzida DECIMAL(15,2)`,
    ...
    // ── ESPACO DE ID COMPARTILHADO ────────────────────────────────────────────
    //
    // As duas tabelas passam a tirar id da MESMA sequencia. Um id de cobranca do
    // ato e unico independente da forma: nao existe boleto #5001 e cartao #5001.
    //
    // Por que isto, em vez de fundir as tabelas: fundir significaria mexer em 22
    // arquivos (o controller de 1067 linhas, o gerador de 1300), migrar 1.114
    // linhas de producao e refazer as cadeias de substituicao e os eventos - no
    // caminho do dinheiro. A sequencia compartilhada resolve o UNICO problema
    // pratico que a separacao criava (ids que se cruzam, e uma acao acertar o
    // registro de outra reserva) em tres comandos reversiveis.
    //
    // Comeca em 10000, acima do maior id de boleto hoje (1202) e com folga. Isso
    // da uma propriedade util de leitura: id abaixo de 10000 e boleto legado, id
    // acima nasceu depois da unificacao. E so funciona limpo porque a tabela do
    // cartao estava VAZIA - nao ha colisao retroativa para resolver.
    //
    // `CREATE SEQUENCE IF NOT EXISTS ... START WITH` e idempotente: o START so
    // vale na criacao, entao rodar todo boot nao reposiciona a sequencia.
    `CREATE SEQUENCE IF NOT EXISTS ato_cobranca_id_seq START WITH 10000`,
    `ALTER TABLE boleto_history        ALTER COLUMN id SET DEFAULT nextval('ato_cobranca_id_seq')`,
    `ALTER TABLE userede_link_history  ALTER COLUMN id SET DEFAULT nextval('ato_cobranca_id_seq')`,

    // ── Timeline do link (append-only, espelho do boleto_events) ──────────────
    `CREATE TABLE IF NOT EXISTS userede_link_events (
        id SERIAL PRIMARY KEY,
        link_history_id INTEGER NOT NULL,
        idreserva INTEGER NOT NULL,
    ...
    `CREATE INDEX IF NOT EXISTS idx_userede_link_events_hist ON userede_link_events (link_history_id)`,
    `CREATE INDEX IF NOT EXISTS idx_userede_link_events_res  ON userede_link_events (idreserva)`,

    // ── Trava de uso do portal (uma operação por vez) ─────────────────────────
    // Sem ela, dois acionamentos simultâneos abrem duas sessões no mesmo
    // usuário, gravam storageState por cima um do outro e podem gerar dois
    // logins - e cada login e uma chance de esbarrar no reCAPTCHA.
    `CREATE TABLE IF NOT EXISTS userede_lock (
    ...
    `INSERT INTO userede_lock (id, created_at, updated_at)
