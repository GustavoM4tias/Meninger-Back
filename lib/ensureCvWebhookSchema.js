...
// ── Vigilância de silêncio (2026-09-10) ──────────────────────────────────────
//
// Webhook não quebra com erro: ele para de chegar. Até aqui `last_event_at` só
// aparecia na tela, então "o CV parou de avisar" era descoberto por acidente,
// dias depois. Estas duas colunas são o que permite AVISAR:
//
//   alerta_silencio_horas  teto de horas sem evento, por funcionalidade (a
//                          operação ajusta na tela; NULL = fallback do código).
//   silencio_alertado_em   um aviso por episódio, como o `alert_sent_at` da
//                          credencial do CV. O evento que volta limpa.
//
// Exportadas porque a PRODUÇÃO RODA COM SKIP_DB_SYNC=true e a fase de schema do
// boot não acontece: quem garante estas colunas de verdade é
// services/cv/cvWebhookHealthService.js, on-demand, no mesmo padrão que o
// MarketingConfigService usa para as colunas dele. Aqui elas ficam para o boot
// completo (dev / FORCE_DB_SYNC=true) e para a definição viver num lugar só.
export const SILENCIO_STATEMENTS = [
    `ALTER TABLE cv_webhook_endpoints
        ADD COLUMN IF NOT EXISTS alerta_silencio_horas INTEGER`,
    `ALTER TABLE cv_webhook_endpoints
        ADD COLUMN IF NOT EXISTS silencio_alertado_em TIMESTAMPTZ`,
    // Preencher vazio pode ficar no ensure normal (CLAUDE.md): não TROCA valor
    // escolhido na tela, só dá um teto inicial a quem ainda não tem nenhum.
    ...FUNCIONALIDADES.map(f =>
        `UPDATE cv_webhook_endpoints
            SET alerta_silencio_horas = ${Number(f.silencio_horas) || 24}
          WHERE funcionalidade = '${f.key}'
            AND alerta_silencio_horas IS NULL`),
];
...
export async function ensureCvWebhookSchema() {
    ...
    for (const sql of [...STATEMENTS, ...SILENCIO_STATEMENTS]) {
    ...
    for (const f of FUNCIONALIDADES) {
        try {
            await db.sequelize.query(`
                INSERT INTO cv_webhook_endpoints
                       (funcionalidade, active, processa, token, descricao, alerta_silencio_horas)
                     VALUES (:key, false, :processa, :token, :descricao, :silencio)
            ...
            `, {
                replacements: {
                    ...
                    silencio: Number(f.silencio_horas) || 24,
                },
            });
        } catch (err) {
