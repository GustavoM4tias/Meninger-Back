...
export async function ensureEmeAtendeOpenerTemplates() {
    ...
    for (const def of EME_ATENDE_OPENER_TEMPLATES) {
        // Rascunho: copy ainda em revisão. Template aprovado é IMUTÁVEL, então
        // subir texto não revisado obriga a excluir e recriar com nome novo.
        if (def.draft) {
            console.log(`${tag} "${def.name}" está como rascunho (draft) - não enviado à Meta.`);
            continue;
        }
        try {
