...
/**
 * Reservas mortas (canceladas/distratadas/vencidas) dentro de uma lista de ids.
 *
 * Lê da tabela local `reservas` — mesma fonte da etapa CV, sem chamada à API.
 * Quais situações contam vem de `boleto_settings.cv_situacoes_reserva_morta`,
 * porque id de situação é dado do tenant, não constante de código.
 *
 * @param {number[]} idreservas
 * @param {number[]} idsSituacaoMorta
 * @returns {Promise<Set<number>>}
 */
export async function fetchReservasMortas(idreservas, idsSituacaoMorta) {
    const ids = [...new Set((idreservas || []).map(Number).filter(Number.isFinite))];
    const sits = [...new Set((idsSituacaoMorta || []).map(Number).filter(Number.isFinite))];
    if (!ids.length || !sits.length) return new Set();
    const rows = await db.sequelize.query(
        `SELECT r.idreserva
           FROM reservas r
          WHERE r.idreserva IN (:ids)
            AND (r.situacao->>'idsituacao')::int IN (:sits)`,
        { replacements: { ids, sits }, type: db.Sequelize.QueryTypes.SELECT },
    );
    return new Set(rows.map(r => Number(r.idreserva)));
}
