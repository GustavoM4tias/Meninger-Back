...
const STATEMENTS = [
  ...
  // ── eme_generated_report_memories ──────────────────────────────────────────
  `CREATE TABLE IF NOT EXISTS eme_generated_report_memories (
      id            UUID PRIMARY KEY,
      user_id       INTEGER   NOT NULL,
      report_id     UUID,
      text          TEXT      NOT NULL,
      source        VARCHAR(10) NOT NULL DEFAULT 'eme',
  ...
  `CREATE INDEX IF NOT EXISTS eme_generated_report_memories_user_idx   ON eme_generated_report_memories (user_id)`,
  `CREATE INDEX IF NOT EXISTS eme_generated_report_memories_report_idx ON eme_generated_report_memories (report_id)`,

  // ── eme_generated_report_dismissals ────────────────────────────────────────
  `CREATE TABLE IF NOT EXISTS eme_generated_report_dismissals (
      id          UUID PRIMARY KEY,
      report_id   UUID    NOT NULL,
      user_id     INTEGER NOT NULL,
      created_at  TIMESTAMP NOT NULL DEFAULT NOW()
  )`,
  `CREATE UNIQUE INDEX IF NOT EXISTS eme_generated_report_dismissals_uq   ON eme_generated_report_dismissals (report_id, user_id)`,
  `CREATE INDEX IF NOT EXISTS eme_generated_report_dismissals_user_idx    ON eme_generated_report_dismissals (user_id)`,

  // Colunas adicionadas depois da 1ª versão (idempotente)
  `ALTER TABLE eme_generated_reports ADD COLUMN IF NOT EXISTS theme VARCHAR(20) NOT NULL DEFAULT 'classic'`,
  `ALTER TABLE eme_generated_reports ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP`,
  `ALTER TABLE eme_generated_reports ADD COLUMN IF NOT EXISTS deleted_by INTEGER`,
  // briefing: pedido original do chat, gravado pela tool create_report. Sem esta
  // coluna a tool falha inteira e a Eme responde "o construtor está instável".
  `ALTER TABLE eme_generated_reports ADD COLUMN IF NOT EXISTS briefing TEXT`,
  `CREATE INDEX IF NOT EXISTS eme_generated_reports_deleted_idx ON eme_generated_reports (deleted_at)`,
