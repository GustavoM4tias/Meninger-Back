...
export const PARCELAS_DEFAULTS = Object.freeze({
    ...
    criterioSienge: 'titulo_e_venda',  // 'titulo_e_venda' | 'titulo' | 'venda' (ver siengeAssumiu)
...
/**
 * O Sienge assumiu a cobranca? Dois sinais no contrato local (`contracts`):
 *   titulo  `receivable_bill_id`          o Financeiro gerou os titulos
 *   venda   `financial_institution_date`  "faturado como venda", a mesma regra
 *                                         do relatorio de Faturamento
 * Medido em 07/09/2026: 42 contratos tinham titulo SEM venda faturada - so o
 * titulo nao basta. Criterio configuravel (`parcelas_criterio_sienge`):
 *   'titulo_e_venda' (padrao) | 'titulo' | 'venda'
 */
export function siengeAssumiu(contrato, criterio = 'titulo_e_venda') {
    if (!contrato) return false;
    if (String(contrato.situation || '').toLowerCase() === 'cancelado') return false;
    const titulo = contrato.receivable_bill_id != null;
    const venda = !!contrato.financial_institution_date;
    if (criterio === 'titulo') return titulo;
    if (criterio === 'venda') return venda;
    return titulo && venda;
}
 ...
 */
export function motivoEncerramento({ contrato, reservaCancelada, encerrarQuandoFaturado = true, situacaoMorta = false, criterio = 'titulo_e_venda' }) {
    if (reservaCancelada || situacaoMorta) return 'reserva_cancelada';
    if (encerrarQuandoFaturado && siengeAssumiu(contrato, criterio)) return 'sienge_faturado';
    return null;
}
...
export default {
    ...
    decidirParcela, condicaoDeEmissao, siengeAssumiu, motivoEncerramento, rotuloParcela, descricaoParcela,
};
