...
export const PARCELAS_DEFAULTS = Object.freeze({
    ...
    idseries: [20],
    // Empreendimentos FORA da cobranca de parcelas: ids do CV (idempreendimento_cv
    // da unidade da reserva). Por ID, nao por nome: o empreendimento muda de nome
    // no CV (PARK ALAMEDA virou PARK ALAMEDA - SARANDI, mesmo id 36) e a tela
    // mostra o nome mais recente. O ato tem o webhook por empreendimento no CV;
    // a parcela nao vem de webhook - a adesao pega toda reserva com ato pago, e
    // esta lista e o unico lugar para tirar um empreendimento (Gustavo, 09/09/2026).
    empreendimentosExcluidos: [],
    exigirAtoPago: true,
    antecedenciaDias: 10,
    ...
    avisoEncerramento: true,           // avisa o cliente quando o plano encerra por venda faturada / etapa do repasse
...
/** O empreendimento (id do CV) esta na lista de exclusao da cobranca de parcelas? */
export function empreendimentoExcluido(idempreendimento, lista = []) {
    const id = Number(idempreendimento);
    return Number.isInteger(id) && id > 0 && (lista || []).some(x => Number(x) === id);
}

const MESES = ['janeiro', 'fevereiro', 'março', 'abril', 'maio', 'junho',
    'julho', 'agosto', 'setembro', 'outubro', 'novembro', 'dezembro'];

/** "outubro/2026" a partir do vencimento (YYYY-MM-DD). */
export function mesAnoParcela(vencimento) {
 ...
 *
 * `porMes` (plano com `numeracao_oculta`, decisao do Gustavo em 10/09/2026):
 * o cliente le "parcela de outubro/2026" em vez de "parcela 3 de 60". Vale para
 * quem tem parcela anterior que o Office nunca cobrou (retroativa fora do corte
 * de 08/09): o numero da sequencia entregaria uma divida que ninguem cobrou.
 * So muda o texto do CLIENTE - mensagem no CV, evento e tela seguem numerando.
 * Sem vencimento na mao, cai no numero (nunca fica sem descricao).
 */
export function rotuloParcela(p, { porMes = false } = {}) {
    const mes = porMes ? mesAnoParcela(p?.vencimento) : null;
    return mes || `${p.numero}/${p.total}`;
}
export function descricaoParcela(p, { porMes = false } = {}) {
    const mes = porMes ? mesAnoParcela(p?.vencimento) : null;
    return mes ? `parcela de ${mes}` : `parcela ${p.numero} de ${p.total}`;
}
...
export default {
    ...
    ehErroDeCep, titularComEnderecoContingencia, rotuloParcela, descricaoParcela, mesAnoParcela,
    empreendimentoExcluido,
};
