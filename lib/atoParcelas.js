...
// intervalo - e PARAR sozinho quando o contrato e emitido (etapa do repasse
// no CV). O Sienge (contrato, titulo, venda faturada) NAO e criterio desde
// 08/09/2026: o titulo pode ser adiantamento e nao prova que o ERP cobra.
//
...
export const PARCELAS_DEFAULTS = Object.freeze({
    ...
    exigirAtoPago: true,
    antecedenciaDias: 10,
    // UNICA regra de encerramento alem da reserva cancelada (Gustavo, 08/09/2026,
    // noite): as etapas do REPASSE no CV.
...
/**
 * O plano deve encerrar? Duas regras, qualquer uma basta:
 ...
 * O Sienge saiu do criterio em 08/09/2026 (Gustavo): ter contrato ou titulo no
 * ERP nao prova que ele cobra as mensais (o titulo pode ser adiantamento), e a
 * venda faturada ja e uma etapa do repasse (54 "Faturado SIENGE MCMV").
 * `sienge_faturado` segue existindo so como motivo HISTORICO nos planos
 * encerrados antes disso.
 */
export function motivoEncerramento({
    reservaCancelada = false, situacaoMorta = false,
...
/** Motivos em que o contrato passa a cobrar: previstas viram `transferida`, nao `cancelada`. (`sienge_faturado` e historico.) */
...
export default {
    ...
    decidirParcela, classificarParaRodada, condicaoDeEmissao, proximoDiaUtil, repasseAssumiu, motivoEncerramento, MOTIVOS_TRANSFERENCIA,
