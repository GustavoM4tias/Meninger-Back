...
export const PARCELAS_DEFAULTS = Object.freeze({
    ...
    idseries: [20],
    // Empreendimentos FORA da cobranca de parcelas (nomes como estao na reserva do
    // CV, sem distinguir maiusculas). O ato tem o webhook por empreendimento no
    // CV; a parcela nao vem de webhook - a adesao pega toda reserva com ato pago,
    // e este e o unico lugar para tirar um empreendimento (Gustavo, 09/09/2026).
...
/** Nome de empreendimento normalizado para comparar (maiusculas, sem espacos sobrando). */
export function normalizarEmpreendimento(nome) {
    return String(nome || '').trim().toUpperCase().replace(/\s+/g, ' ');
}

/** O empreendimento esta na lista de exclusao da cobranca de parcelas? */
export function empreendimentoExcluido(nome, lista = []) {
    const n = normalizarEmpreendimento(nome);
    return !!n && (lista || []).some(x => normalizarEmpreendimento(x) === n);
}
...
export default {
    ...
    normalizarEmpreendimento, empreendimentoExcluido,
};
