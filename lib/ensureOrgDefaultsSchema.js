...
//
// Saíram em 2026-08-27 (ver lib/ensureOrgRetirement.js, que apaga do banco):
// "Financeiro" (virou Contas a Pagar + Contas a Receber), "Sócio Fundador"
// (já inativo; o CARGO segue existindo na Diretoria), "Jurídico", "Recursos
// Humanos" e "Tecnologia da Informação" — a empresa não tem essas áreas hoje.
// Recolocar qualquer um aqui faz o seed recriá-lo no boot seguinte.
const DEFAULT_DEPARTMENTS = [
    ...
    ['CONTAS_A_PAGAR', 'Contas a Pagar', 'Títulos a pagar, custos por centro de custo, conciliação com o Sienge e relacionamento com fornecedores.'],
    ['CONTAS_A_RECEBER', 'Contas a Receber', 'Cobrança do ato, boletos e link de cartão, consulta de nº CEF, conciliação de recebimentos e inadimplência.'],
...
//
// Os cargos "…Financeiro" seguem existindo (é o cargo real de quem está lotado
// neles) e passaram para Contas a Receber junto com o departamento em
// 2026-08-27. Advogado, Analista de RH e Analista de TI saíram com os
// departamentos deles. O cargo "Sócio Fundador" ficou, agora na Diretoria.
const DEFAULT_POSITIONS = [
    ['SOCIO_FUNDADOR', 'Sócio Fundador', 'DIRETORIA', 0, 'Sócio fundador da empresa.'],
    ...
    ['ANALISTA_CONTAS_PAGAR', 'Analista de Contas a Pagar', 'CONTAS_A_PAGAR', 5, 'Títulos, custos e conciliação de pagamentos com o Sienge.'],
    ['ASSISTENTE_CONTAS_PAGAR', 'Assistente de Contas a Pagar', 'CONTAS_A_PAGAR', 6, 'Apoio às rotinas de contas a pagar.'],
    ['GERENTE_FINANCEIRO', 'Gerente Financeiro', 'CONTAS_A_RECEBER', 2, 'Gestão financeira: caixa, pagamentos e recebimentos.'],
    ['ANALISTA_FINANCEIRO', 'Analista Financeiro', 'CONTAS_A_RECEBER', 5, 'Rotinas de contas a pagar/receber e conciliações.'],
    ['ASSISTENTE_FINANCEIRO', 'Assistente Financeiro', 'CONTAS_A_RECEBER', 6, 'Apoio às rotinas financeiras.'],
...
async function seedDepartments() {
    ...
    for (const [code, name, description] of DEFAULT_DEPARTMENTS) {
        ...
        if (found) {
            // Descrições do conjunto padrão são CURADAS pelo seed (decisão
            // 2026-07-29): atualiza também registros pré-existentes com esses
            // nomes. Nome/código do admin nunca são tocados.
            if (description && found.description !== description) await found.update({ description });
            continue;
        }
...
async function seedPositions(deptByKey) {
    ...
    for (const [code, name, deptCode, level, description] of DEFAULT_POSITIONS) {
        const found = byKey.get(norm(code)) || byKey.get(norm(name));
        if (found) {
            // Mesmo critério dos departamentos: descrição e nível do conjunto
            // padrão são curados pelo seed, inclusive em registros antigos.
            const patch = {};
            if (found.level !== level) patch.level = level;
            if (description && found.description !== description) patch.description = description;
