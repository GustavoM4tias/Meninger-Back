...
// /aprovacoes saiu em 2026-08-19 (módulo removido). /settings/organograma
// deixou de ser transversal na mesma data: por decisão, hoje só o Comercial vê.
const COMMON_ROUTES = [
    '/checklists',
    '/microsoft/teams',
];

const MARKETING = ['/marketing/events', '/marketing/leads', '/marketing/stand-vendas', '/marketing/viabilidade', '/marketing/projecao-investimentos'];
...
const RELATORIOS = [
    ...
    '/comercial/relatorios/precadastros', '/comercial/relatorios/reservas',
    '/comercial/relatorios/leads', '/comercial/relatorios/imobiliarias',
    '/comercial/relatorios/corretores',
];
const COMERCIAL = [
    ...RELATORIOS, '/comercial/projections', '/comercial/conditions',
    '/comercial/mcmv', '/crm/workflow/groups', '/crm/buildings', '/crm/imobiliarias',
    // Organograma é alçada do Comercial desde 2026-08-19 (era transversal).
    '/settings/organograma',
    // Cancelamento de Reservas deixou de ser admin-only em 2026-08-19: o
    // histórico e o reprocessamento são do Comercial (config segue admin).
    '/comercial/cancelamento-reservas',
];
// /financeiro/paymentflow FORA de propósito: a tela está inativa e, por decisão
// de 2026-08-19, não vai em perfil nenhum. Recolocar aqui reconcede a todos os
// perfis não customizados no boot seguinte.
const CONTAS_PAGAR = ['/financeiro/titulos', '/financeiro/custos'];
const CONTAS_RECEBER = ['/financeiro/consulta-cef', '/financeiro/cobranca/ato'];
// Conjunto guarda-chuva de quem só CONSULTA o financeiro (Diretoria, apoio).
// Mantido igual ao de antes da divisão em Contas a Pagar / Contas a Receber
// para não conceder tela nova a perfil que ninguém pediu.
const FINANCEIRO = [...CONTAS_PAGAR, '/financeiro/consulta-cef'];
const MICROSOFT_EXTRA = ['/microsoft/sharepoint', '/microsoft/planner'];

// code do departamento (ensureOrgDefaultsSchema) → telas da área.
// FINANCEIRO, JURIDICO, RH, TI e SOCIO_FUNDADOR saíram em 2026-08-27 junto com
// os departamentos (lib/ensureOrgRetirement.js).
const DEFAULT_PROFILE_ROUTES = {
    ...
    MARKETING: [...MARKETING, '/crm/buildings', '/comercial/relatorios/projecao'],
    CONTAS_A_PAGAR: [...CONTAS_PAGAR, '/marketing/viabilidade', '/comercial/relatorios/faturamento'],
    CONTAS_A_RECEBER: [...CONTAS_RECEBER, '/comercial/relatorios/faturamento', '/comercial/conditions', '/validator'],
    CONTABILIDADE: ['/financeiro/titulos', '/financeiro/custos', '/comercial/relatorios/faturamento'],

    // Áreas de apoio: só o que realmente consomem.
    ADMINISTRATIVO: ['/financeiro/titulos', ...MICROSOFT_EXTRA],
    SUPRIMENTOS: ['/financeiro/titulos', '/financeiro/custos'],
    ENGENHARIA: ['/financeiro/custos', '/crm/buildings', '/marketing/stand-vendas'],
    NOVOS_NEGOCIOS: ['/marketing/viabilidade', '/crm/buildings', '/comercial/relatorios/projecao'],
    LEGALIZACAO: ['/crm/buildings', '/comercial/relatorios/precadastros', '/validator'],
    ASSISTENCIA_TECNICA: ['/crm/buildings'],
};

// Departamento criado à mão pelo admin (fora do conjunto padrão): casa por
// palavra-chave do nome para não nascer só com o comum. TODAS as que casarem
// somam (não é a primeira que ganha) — por isso cada regex precisa ser justa.
// `\bobras?\b` e não `obra`: "Cobrança" contém "obra" e estava levando as telas
// de Engenharia junto.
const ROUTES_BY_DEPT_KEYWORD = [
    ...
    [/receb|cobran/i, DEFAULT_PROFILE_ROUTES.CONTAS_A_RECEBER],
    [/pagar|t[ií]tulo|fornecedor/i, DEFAULT_PROFILE_ROUTES.CONTAS_A_PAGAR],
    [/financ|cont[aá]b|fiscal/i, DEFAULT_PROFILE_ROUTES.CONTABILIDADE],
    [/\bobras?\b|engenharia|t[eé]cnic/i, DEFAULT_PROFILE_ROUTES.ENGENHARIA],
    [/suprimento|compra/i, DEFAULT_PROFILE_ROUTES.SUPRIMENTOS],
    [/diretoria|dire[cç][aã]o|s[oó]cio|presid/i, DEFAULT_PROFILE_ROUTES.DIRETORIA],
    [/jur[ií]dic|legaliza/i, DEFAULT_PROFILE_ROUTES.LEGALIZACAO],
];

// O código do departamento vem do cadastro e nem sempre bate com a chave do
// mapa: "Novos Negócios" está gravado como `NOVOS NEGÓCIOS` (com espaço e
// acento) e por isso caía no genérico, nascendo só com as telas comuns.
const chaveDeCodigo = (code) => String(code || '')
...
export function defaultRoutesForDepartment(dept) {
    ...
    const byCode = DEFAULT_PROFILE_ROUTES[chaveDeCodigo(dept?.code)];
    if (byCode) {
