...
const STATEMENTS = [
    ...
    // ── Sessão Microsoft: MARCAR em vez de APAGAR ────────────────────────────
    //
    // Até 25/08/2026, quando o backend concluía que a autorização tinha morrido
    // ele APAGAVA o refresh_token. Isso é irreversível: se a conclusão estivesse
    // errada - e esteve, várias vezes - a pessoa era ejetada sem volta possível,
    // porque o único artefato capaz de recuperar a sessão tinha sido destruído.
    //
    // Agora a suspeita vira MARCA e o token FICA. A tela pede reconexão do mesmo
    // jeito, mas o sistema continua tentando renovar por baixo: se a recusa era
    // passageira ou se a classificação errou, a sessão se cura sozinha e a
    // pessoa nunca fica sabendo. Apagar de verdade só no desvincular explícito.
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS microsoft_reauth_required BOOLEAN NOT NULL DEFAULT FALSE`,
    // Guardado para diagnóstico: sem isto, o motivo real da queda vivia só no
    // log do processo e sumia no primeiro restart - foi o que impediu achar a
    // causa por horas.
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS microsoft_auth_error TEXT`,
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS microsoft_auth_error_at TIMESTAMP WITH TIME ZONE`,
];
