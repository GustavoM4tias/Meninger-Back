...
function resolveAudiencesForWrite(input) {
    ...
    if (!arr.length) return visibilityToAudiences('INTERNAL'); // padrão seguro: interno
...
// Resolve userIds afetados por um escopo (mesma lógica das trilhas).
...
async function loadAssignments(comunicadoId) {
    return db.AcademyComunicadoAssignment.findAll({
        where: { comunicadoId },
...
async function ackStats(comunicadoId) {
    const recipients = await db.AcademyComunicadoReceipt.count({ where: { comunicadoId } });
    const acked = await db.AcademyComunicadoReceipt.count({ where: { comunicadoId, ackedAt: { [Op.ne]: null } } });
    return { recipients, acked, pending: recipients - acked };
}

// Notificação ao publicar (background).
async function notifyComunicado(row, userIds) {
    ...
    await NotificationService.notify({
        type: NotificationType.ACADEMY_COMUNICADO,
...
const comunicadoAdminService = {
    async list({ status } = {}) {
        ...
        const rows = await db.AcademyComunicado.findAll({
            where,
    ...
    async get({ id }) {
        const row = await db.AcademyComunicado.findByPk(Number(id));
    ...
    async create({ payload, userId }) {
        ...
        const created = await db.AcademyComunicado.create({
            title,
    ...
    async setAssignments({ id, assignments }) {
        const row = await db.AcademyComunicado.findByPk(Number(id), { attributes: ['id'] });
        if (!row) throw new Error('Comunicado não encontrado.');
        ...
        await db.sequelize.transaction(async (t) => {
            await db.AcademyComunicadoAssignment.destroy({ where: { comunicadoId: row.id }, transaction: t });
            if (norm.length) await db.AcademyComunicadoAssignment.bulkCreate(norm, { transaction: t });
        });
    ...
    async publish({ id, userId }) {
        ...
        await db.sequelize.transaction(async (t) => {
            ...
            await db.AcademyComunicadoReceipt.bulkCreate(recs, { ignoreDuplicates: true, transaction: t });
        ...
        notifyComunicado(row, userIds).catch((err) => console.warn('[academy.comunicado.publish] notify failed', err?.message));
    ...
    async remove({ id }) {
        ...
        await db.sequelize.transaction(async (t) => {
            await db.AcademyComunicadoAssignment.destroy({ where: { comunicadoId: row.id }, transaction: t });
            await db.AcademyComunicadoReceipt.destroy({ where: { comunicadoId: row.id }, transaction: t });
    ...
    async adherence({ id }) {
        const row = await db.AcademyComunicado.findByPk(Number(id), { attributes: ['id', 'title', 'requiresAck', 'status'] });
        if (!row) throw new Error('Comunicado não encontrado.');
        const receipts = await db.AcademyComunicadoReceipt.findAll({
